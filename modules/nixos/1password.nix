...

{
  ...
  programs._1password-gui = lib.mkIf (pkgs.stdenv.isLinux && config.myOptions.hasGui) {
    enable = true;
  ...
  systemd.user.services."1password" = lib.mkIf (pkgs.stdenv.isLinux && config.myOptions.hasGui) {
