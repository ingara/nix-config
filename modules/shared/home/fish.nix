  ...
  programs.fish = {
    ...
    functions = {
      ...
      zi = ''
        ...
        and cd $result
      '';
      # Runs once after plugin/integration binds are loaded, so user
      # overrides win. Used here only to neutralise the upstream fzf
      # shell integration's Alt+C (\ec) binding: Ghostty's Cmd+C falls
      # through as Meta+C when there's no selection (copy_to_clipboard
      # returns false in Surface.zig), and we don't want that silent
      # fall-through to open fzf-cd-widget. Alt+D (∂) is the user's
      # actual fzf-cd entry point on macOS — defined in shellInit above.
